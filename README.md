...
6. **Gemini API Integration**
   ...
   - The project now uses Google's Gemini API for generating AI responses. To use this feature:
     ...
     1. **Setting up your Gemini API Key**:
        ...
        - Set it as an environment variable:
          ...
          ```
        - Or update in the docker-compose files

     2. **Gemini API Endpoints**:

        - `/api/chat` - Standard endpoint that utilizes Gemini within the RAG framework
        - `/api/gemini` - Direct access to Gemini API (bypasses RAG infrastructure)

     3. **Configuring Gemini**:
        - Adjust temperature, max tokens and other settings in `src/ai/core/config.py` or via environment variables

     Note: The default API key is for testing only with limited quota. For production use, please use your own API key.
