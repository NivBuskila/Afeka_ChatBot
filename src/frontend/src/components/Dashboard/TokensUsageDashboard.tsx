...
export const TokenUsageAnalytics: React.FC<TokenUsageAnalyticsProps> = ({
  ...
  const handleExportCSV = () => {
    ...
    const csvData = Object.entries(usageData.keys).map(([, key], index) => ({
      "Key ID": `Key #${index + 1}`,
      Status: key.status,
      ...
      "Tokens Remaining":
        usageData.limits.max_tokens_per_day - key.tokens_used_today,
      "Daily Usage %": Math.round(
        (key.tokens_used_today / usageData.limits.max_tokens_per_day) * 100
      ),
    }));
  ...
  const keyData = Object.entries(usageData.keys).map(([, key], index) => ({
    id: index + 1,
    name: `Key #${index + 1}`,
  ...
  const blockedKeys = keyData.filter(
    (k) => k.status === "blocked" || k.status === "error"
  ).length;
  const totalRequestsToday = Object.values(usageData.keys).reduce(
    (sum, key) => sum + key.requests_today,
    0
  );
