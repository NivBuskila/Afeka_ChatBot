    ...
    <div className="bg-white/80 dark:bg-black/30 backdrop-blur-lg border border-gray-300 dark:border-green-500/20 rounded-lg p-6 shadow-lg">
      <div
        ...
        className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors
          ${isDragActive ? 'border-green-500 bg-green-500/10' : 'border-gray-400 dark:border-green-500/30 hover:border-green-500 dark:hover:border-green-500/50'}`}
      >
        <input {...getInputProps()} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {['PDF', 'DOC', 'DOCX', 'XLS', 'XLSX', 'TXT'].map((type) => (
            ...
            >
              ...
              <span className="text-sm text-gray-700 dark:text-green-400/80">{type}</span>
            </div>
          ))}
        </div>
      </div>
