...
const MessageSearchBar: React.FC<MessageSearchBarProps> = ({
  ...
  const { rightClass, marginRight } = useRTL();
  const { direction, textAlign } = useTextDirection(searchQuery);

  return (
    <div
      className="p-2 bg-gray-100 dark:bg-black flex items-center flex-shrink-0"
      dir={direction}
    >
      <div className="relative flex-1 max-w-md mx-auto">
        <input
          ...
          className={`w-full py-1 px-3 ${rightClass} text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent ${textAlign}`}
          autoFocus
        />
        <div
        ...
        >
          {searchResults.length > 0 && (
            <span
              className={`text-xs text-gray-500 dark:text-gray-400 ${marginRight}`}
            >
