...
class ContextBuilder:
    ...
    def __init__(self):
        # תבניות prompt לפי סוג השאלה
        self.prompt_templates = {
            'temporal': self._get_temporal_template(),
            'procedural': self._get_procedural_template(),
            'definitional': self._get_definitional_template(),
    ...
    def _build_instructions(self, query_type: str, query: str) -> str:
        ...
        # זיהוי שאלות מורכבות או רב-שלביות
        if any(word in query.lower() for word in ['וגם', 'בנוסף', 'כמו כן', 'באיזה מקרים']):
            base_instructions.append("8. זוהתה שאלה מורכבת - ענה על כל החלקים בנפרד")
