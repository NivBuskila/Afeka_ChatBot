...
describe('Authentication and Chat Flow Integration', () => {
  ...
  describe('Session Management', () => {
    ...
    it('should handle session with messages', async () => {
      ...
      mockChatService.getCurrentUser.mockResolvedValue(mockUser);
      mockChatService.getChatSessionWithMessages.mockResolvedValue(
      ...
      const sessionWithMessages = await mockChatService.getChatSessionWithMessages(
        'session-123'
      );

      expect(sessionWithMessages).toEqual(mockSessionWithMessages);
      expect(sessionWithMessages.messages).toHaveLength(2);
      expect(sessionWithMessages.messages[0].is_bot).toBe(false);
      expect(sessionWithMessages.messages[1].is_bot).toBe(true);
    });
  });
